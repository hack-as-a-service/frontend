...
it("disables the button when the input does not contain the app name", () => {
  mountChakra(<ConfirmDelete
                ...
                onClose={empty} />);
  cy.waitForReact();
  cy.react("ConfirmDelete").get("button").contains(buttonText).should("be.disabled");
});
...
it("enables the button when the input contains the app name", () => {
  ...
  cy.react("ConfirmDelete").get("button").contains(buttonText).should("not.be.disabled");
});
...
it("calls the confirmation callback", () => {
  ...
  cy.waitForReact();
  cy.react("ConfirmDelete").get("input").type(name);
  cy.react("ConfirmDelete").get("button").contains(buttonText).click().then(() => expect(fns.confirm).to.be.calledOnce);
});
